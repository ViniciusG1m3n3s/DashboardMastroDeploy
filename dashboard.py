    ...
    if opcao_selecionada == "Visão Geral":
        ...
        with col1:      
            with st.container(border=True):
                ...
                fig_produtividade = px.line(
                    ...
                    line_shape='linear',  # Isso cria linhas segmentadas entre os pontos
        ...
        with col2:
            with st.container(border=True):
                ...
                fig_tmo.update_traces(text=df_tmo['TMO_Formatado'], textposition="top center")  # Exibe o tempo formatado
        ...
        with st.container(border=True):
            ...
            fig_tmo_analista.update_traces(textposition='outside')  # Exibe o tempo formatado fora das barras
        ...
        with col1:
            with st.container(border=True):
                ...
                # Define a posição da legenda abaixo do gráfico
                fig_status_analista.update_layout(
                    legend=dict(
                        orientation="h",  # Orientação horizontal
                        yanchor="top",  # Alinhamento vertical superior
                        y=-0.1,  # Coloca a legenda um pouco abaixo do gráfico
